...
namespace TelegramBotApiBundle\Controller;
...
use TelegramBotApiBundle\UpdateReceive\UpdateReceiveInterface;
...
class DefaultController extends Controller
{
    ...
    {
        $api = $this->get("telegram_bot_api");
        $config = $this->getParameter("telegram_bot_api.config");

        if (empty($config['webhook']['domain'])) {
            throw new Exception("'telegram_bot_api.webhook.domain' is not set in config.yml", 0);
        }
